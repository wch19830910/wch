...
-----------------
[rewrite_local]
https:\/\/qfapi\.hmting\.com\/store\/assign\/sign url script-request-header https://raw.githubusercontent.com/wch19830910/wch/main/QuantumultX/hmt.js

[task_local]
5 0 * * * https://raw.githubusercontent.com/wch19830910/wch/main/QuantumultX/hmt.js, tag=鹤鸣亭

【All App MitM】
hostname = qfapi.hmting.com


const $ = new API("hmt", true);
const ERR = MYERR();
$.cookie = $.read("evil_hmtCookie");
...
!(async () => {
  ...
  if ($.cookie != undefined && $.usertoken != undefined) {
    ...
    $.notify("鹤鸣亭", "", "❌ 请先获取Cookie");
  }
})()
  .catch((err) => {
    if (err instanceof ERR.ParseError) {
      $.notify("鹤鸣亭", "❌ 解析数据出现错误", err.message);
    } else if (err instanceof ERR.EventError) {
      $.notify("鹤鸣亭", "❌ 请尝试重新获取Cookie", err.message);
    } else {
      $.notify(
        "鹤鸣亭",
        "❌ 出现错误",
...
function checkin() {
  ...
  const headers = {
    ...
    Referer: `https://qfapi.hmting.com/store-view/store/home`,
...
function checkinfo() {
  const url2 = `https://qfapi.hmting.com/store/assign/index`;
  const headers2 = {
    Origin: `https://qfapi.hmting.com`,
    Cookie: $.cookie,
    "Client-Platform": `APP-IOS`,
...
function showmsg() {
  ...
  if ($.data.isSign != null && $.data.isSign == true) {
    $.notify("鹤鸣亭", "今日已签到🎉", `累计签到${count}天！`);
  } else if ($.data.isSign != null && $.data.isSign == false) {
    point = $.data.point;
    $.notify("鹤鸣亭", "签到成功🎉", `获得${point}积分，累计签到${count}天！`);
  }
}
